...
// 这个文件负责更高级的动态内存管理
// 包含一些基本函数、空间配置器、未初始化的存储空间管理，以及一个模板类auto_otr
...
{
    return get_buffer_helper(len, static_cast<T*>(0)); //帮助进行参数推导
}
...
public:
    ...
    ~temporary_buffer()
    {
        MyStl::destroy(buffer, buffer + len); // 调用缓冲区之间每一个对象的析构
        free(buffer); // 将缓冲区还给操作系统
    }
...
public:
    explicit auto_ptr(T* p = nullptr) : m_ptr(p) {} // 必须显示构造函数
    auto_ptr(auto_ptr& rhs) : m_ptr(rhs.release()) {}
    template <class U>
    auto_ptr(auto_ptr<U> &rhs) : m_ptr(rhs.release()) {} // 针对复杂对象进行 rhs.release() 让rhs指向空，并返回rhs原本的指向

    auto_ptr& operator=(auto_ptr &rhs) //赋值运算符重载
    {
        if (this != &rhs)
        {
            delete m_ptr;
